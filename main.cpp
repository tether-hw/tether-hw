...
std::string proveCircuit(ProverContextT& context, Tether::Circuit* circuit)
{
...
Tether::Circuit* newCircuit(Tether::BlockType blockType, ethsnarks::ProtoboardT& outPb)
{
    switch(blockType)
    {
        case Tether::BlockType::RingSettlement: return new Tether::RingSettlementCircuit(outPb, "circuit");
        case Tether::BlockType::Deposit: return new Tether::DepositCircuit(outPb, "circuit");
        case Tether::BlockType::OnchainWithdrawal: return new Tether::OnchainWithdrawalCircuit(outPb, "circuit");
        case Tether::BlockType::OffchainWithdrawal: return new Tether::OffchainWithdrawalCircuit(outPb, "circuit");
        case Tether::BlockType::InternalTransfer: return new Tether::InternalTransferCircuit(outPb, "circuit");
        default:
        {
...
Tether::Circuit* createCircuit(Tether::BlockType blockType, unsigned int blockSize, bool onchainDataAvailability, ethsnarks::ProtoboardT& outPb)
{
    std::cout << "Creating circuit... " << std::endl;
    auto begin = now();
    Tether::Circuit* circuit = newCircuit(blockType, outPb);
...
bool generateWitness(Tether::Circuit* circuit, const json& input)
{
...
bool validateCircuit(Tether::Circuit* circuit)
{
...
std::string getBaseName(Tether::BlockType blockType)
{
    switch(blockType)
    {
        case Tether::BlockType::RingSettlement: return "trade";
        case Tether::BlockType::Deposit: return "deposit";
        case Tether::BlockType::OnchainWithdrawal: return "withdraw_onchain";
        case Tether::BlockType::OffchainWithdrawal: return "withdraw_offchain";
        case Tether::BlockType::InternalTransfer: return "internal_transfer";
...
void runServer(Tether::Circuit* circuit, const std::string& provingKeyFilename, const libsnark::Config& config, unsigned int port)
{
    ...
    svr.Get("/prove", [&](const Request& req, Response& res) {
        ...
        if (Tether::BlockType(iBlockType) != circuit->getBlockType() || blockSize != circuit->getBlockSize())
        {
...
bool runBenchmark(Tether::Circuit* circuit, const std::string& provingKeyFilename)
{
...
int main (int argc, char **argv)
{
    ...
    if (iBlockType >= int(Tether::BlockType::COUNT))
    {
        std::cerr << "Invalid block type: " << iBlockType << std::endl;
        return 1;
    }
    Tether::BlockType blockType = Tether::BlockType(iBlockType);
    ...
    Tether::Circuit* circuit = createCircuit(blockType, blockSize, onchainDataAvailability, pb);
    if (config.swapAB)
    {
